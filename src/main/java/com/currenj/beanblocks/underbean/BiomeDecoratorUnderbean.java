...
public class BiomeDecoratorUnderbean extends BiomeDecorator
{
    // This predicate is used by WorldGen constructors to define
    // whether a block can be replaced by ore generation.
    Predicate<IBlockState> replaceablePredicate = new UnderbeanPredicate();
    ...
    protected WorldGenFlowersUnderbean flowerGen;

    /**
     * Instantiates a new biome decorator cloud.
     */
    public BiomeDecoratorUnderbean()
    {
        super();
    ...
    private void generateFlowers(World worldIn, Biome biomeIn, Random random, BlockPos chunkPos)
    {
        if(TerrainGen.decorate(worldIn, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.FLOWERS))
            for (int numFlowersGenerated = 0; numFlowersGenerated < flowersPerChunk; ++numFlowersGenerated)
            {
                ...
                flowerGen = new WorldGenFlowersUnderbean();
    ...
    static class UnderbeanPredicate implements Predicate<IBlockState>
    {
        private UnderbeanPredicate()
        {
        }
        ...
        {
            if (parBlockState != null && parBlockState.getBlock() == ModBlocks.beanBlockHardened)
            {
                return true;
            }
            else
