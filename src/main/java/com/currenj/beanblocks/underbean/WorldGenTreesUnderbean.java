...
public class WorldGenTreesUnderbean extends WorldGenAbstractTree
{
    private IBlockState blockStateWood = ModBlocks.blockAncientBeans.getDefaultState();
     ...
     */
    public WorldGenTreesUnderbean(boolean parShouldNotify)
    {
    ...
    public boolean generate(World parWorld, Random parRandom, BlockPos parBlockPos)
    {
        ...
        if (parBlockPos.getY() >= 1 && parBlockPos.getY() + minHeight + 1 <= parWorld.getHeight())
        {
            ...
            else
            {
                ...
                if (parBlockPos.getY() < parWorld.getHeight() - minHeight - 1)
                {
